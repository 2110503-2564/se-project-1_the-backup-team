...
export const getTimeslots = (spaceId: string, roomId: string, date: string) => {
  return new Promise<TimeSlots[]>(async (resolve, reject) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/spaces/${spaceId}/rooms/${roomId}/reservations/timeslots?date=${date}`,
        { cache: 'no-store' },
      )
...
export const addSpace = (space: Partial<Space>, token: string) => {
  return new Promise(async (resolve, reject) => {
    try {
      space.rooms = []
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/spaces/`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
